...
#define MOUSE_SENSITIVITY 0.15f   // degrees per pixel
...
static struct {
    ...
    bool mouse_captured;          // true → cursor hidden & relative movement
    float cam_rx, cam_ry;         // camera yaw / pitch
    vec3_t cam_pos;               // where the camera is
    // float move_speed;
    // float vertical_speed;
} state;
...
static void toggle_mouse_capture(void) {
    state.mouse_captured = !state.mouse_captured;
    sapp_lock_mouse(state.mouse_captured);
    sapp_show_mouse(!state.mouse_captured);
}
...
static void handle_input(const sapp_event* ev) {
    if (ev->type == SAPP_EVENTTYPE_KEY_DOWN) {
        switch (ev->key_code) {
            case SAPP_KEYCODE_ESCAPE:
                if (state.mouse_captured) toggle_mouse_capture();
                break;
            case SAPP_KEYCODE_W:      key_state |=  (1u << 0); break;
            case SAPP_KEYCODE_S:      key_state |=  (1u << 1); break;
            case SAPP_KEYCODE_A:      key_state |=  (1u << 2); break;
            case SAPP_KEYCODE_D:      key_state |=  (1u << 3); break;
            case SAPP_KEYCODE_SPACE:  key_state |=  (1u << 4); break;
            case SAPP_KEYCODE_LEFT_SHIFT:
            case SAPP_KEYCODE_RIGHT_SHIFT: key_state |= (1u << 5); break;
        }
    }
    ...
    else if (ev->type == SAPP_EVENTTYPE_MOUSE_MOVE && state.mouse_captured) {
        state.cam_ry -= ev->mouse_dx * MOUSE_SENSITIVITY;  // yaw: horizontal mouse movement
        state.cam_rx -= ev->mouse_dy * MOUSE_SENSITIVITY;  // pitch: vertical mouse movement
        state.cam_rx  = vm_clamp(state.cam_rx, -89.0f, 89.0f); // pitch clamp (no flip-over)
    }
...
static void update_camera(float dt) {
    ...
    float yaw   = vm_radians(state.cam_ry);
    float pitch = vm_radians(state.cam_rx);
    ...
    vec3_t right = vec3_normalize(vec3_cross(forward, (vec3_t){0,1,0}));
    ...
    if (KEY(4)) delta = vec3_add(delta, vec3_mulf(up,       vspeed)); // Space
    if (KEY(5)) delta = vec3_add(delta, vec3_mulf(up,      -vspeed)); // Shift

    state.cam_pos = vec3_add(state.cam_pos, delta);
}

/* -------------------------------------------------------------
   Forward declarations
   ------------------------------------------------------------- */
static vs_params_t compute_vsparams();
...
static void init(void) {
    ...
    state.cam_pos   = (vec3_t){ 0.0f, 1.5f, 4.0f };
    state.cam_rx    = 0.0f;      // pitch
    state.cam_ry    = 0.0f;      // yaw
    state.move_speed = 5.0f;
    state.vertical_speed = 3.0f;
...
static void frame(void) {
    ...
    update_camera(dt);               // <-- NEW
    // state.rx += …   (remove old auto-spin)

    const vs_params_t vs_params = compute_vsparams();   // <-- NEW
...
static vs_params_t compute_vsparams(void) {
    ...
    mat44_t proj = mat44_perspective_fov_rh(vm_radians(60.0f), w/h, 0.01f, 100.0f);

    /* ---- eye = cam_pos, target = cam_pos + forward ---- */
    float yaw   = vm_radians(state.cam_ry);
    float pitch = vm_radians(state.cam_rx);
    vec3_t forward = {
        cosf(pitch) * sinf(yaw),
    ...
    vec3_t eye    = state.cam_pos;
    vec3_t center = vec3_add(eye, forward);
    vec3_t up     = {0,1,0};

    mat44_t view = mat44_look_at_rh(eye, center, up);
    mat44_t view_proj = vm_mul(view, proj);

    /* ---- cube is now static at the origin ---- */
    mat44_t model = mat44_identity();          // no rotation/translation for the cube
